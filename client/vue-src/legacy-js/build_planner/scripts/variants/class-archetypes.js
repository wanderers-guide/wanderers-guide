...
function applyClassArchetypeChoice(classFeature){
  if(!gOption_hasClassArchetypes){ return null; }
...
function assembleClassArchetypeTabs(tabsID, classFeatureID, originalDescription, srcStruct){
...
function replaceClassFeatureCodeFromClassArchetype(classFeatureID, classFeatureCode, srcStruct){
  if(!gOption_hasClassArchetypes){ return classFeatureCode; }
  ...
  if(g_classArchetypeChosenArchetype.dedicationFeatName != null){
    ...
    if(!gOption_hasFreeArchetype){
      if(wscCode.trim() == 'GIVE-CLASS-FEAT=2' && srcStruct.sourceLevel == 2 && srcStruct.sourceType == 'class'){
        let featSrcStruct = cloneObj(srcStruct);
        featSrcStruct.sourceCodeSNum = 'aa';
        removeUnselectedData(featSrcStruct, false);
