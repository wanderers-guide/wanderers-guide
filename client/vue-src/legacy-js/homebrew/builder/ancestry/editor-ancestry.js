...
$(function () {
    ...
    socket.emit("requestHomebrewAncestryDetails", $('#builder-container').attr('data-bundle-id'));

});

socket.on("returnHomebrewAncestryDetails", function(ancestryObject, featsObject){

    let ancestryMap = objToMap(ancestryObject);
    let featMap = objToMap(featsObject);
    
    let ancestry = ancestryMap.get($('#builder-container').attr('data-ancestry-id'));
