...
$(function () {
    ...
    socket.emit("requestHomebrewItemDetails", $('#builder-container').attr('data-bundle-id'));
...
function finishItem(isUpdate){
    ...
    let itemID = $('#builder-container').attr('data-item-id');
    if(isUpdate){
        requestPacket = "requestHomebrewUpdateItem";
    } else {
        requestPacket = "requestHomebrewAddItem";
    }
...
socket.on("returnHomebrewCompleteItem", function() {
