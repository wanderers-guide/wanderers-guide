...
2. Apply additive prerequisites before dependent functions. Apply
   `20260905000000_edge_work_budget.sql` using the approved database
   ...
   access. Apply `20260921000000_fix_dominion_epithet.sql` in the same phase; it
   replaces only the duplicated nested identifiers for the Trespasser option. Its
   read-only requirement verifies both Dominion options and all four nested
   operation identifiers remain distinct. No historical migration replay is needed.
   Apply `20260924000000_fix_reclaimant_plea.sql` for the three Reclaimant Plea
   records. It repairs their boolean heightening switch and third innate spell,
   preserves saved selection identifiers, and aborts on unexpected content changes
   or pending curator submissions. Its read-only requirement checks all three
   records; it does not modify characters or homebrew.
   Apply `20260925000000_fix_rootball_intelligence.sql` for Rootball Chair's
   Intelligence modifier. It changes only the published template's +4 to -4,
   preserves operation identifiers and other attributes, and aborts on an
   unexpected value or pending curator submission. Its read-only requirement
   verifies the corrected modifier. Existing saved companion copies are untouched.
...
## Quiet character saving

Normal saving, successful retries and compatible remote updates run without a
...
additional server characters. Versioned drafts with a known common ancestor can be
reconciled automatically. Older drafts without enough information for a safe merge
remain untouched; their presence alone does not interrupt the user or replace a
newer server save.

Recovery verification covers reload, account changes and builder/sheet navigation,
with server reads proving saved values and local checks proving unreplayable drafts
are preserved. The Cypress `saveRecovery` scenario also checks that the ordinary
sheet has no added saving controls or notifications at desktop and phone widths.
Connection, encounter and remote-update scenarios verify real acknowledgements,
reopening, and error-only notifications rather than relying on a displayed Saved label.
