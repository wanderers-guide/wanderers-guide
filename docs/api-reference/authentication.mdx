...
## Protected account and moderation writes

Profiles are created by the trusted Auth signup trigger. Account settings, API-client
configuration, Patreon entitlements and GM-group changes go through authenticated
Edge Functions with explicit owner and field selection. Anonymous and authenticated
REST clients cannot insert, update or delete profiles, including extra self-owned
profiles. Role, moderation and entitlement flags are server-managed.

Content submissions are created through `create-content-update`. The server sets the
