...
function onTwoslashError(err, code, lang) {
  console.error(JSON.stringify({ err, code, lang }));
}
function onShikiError(err, code, lang) {
  console.error(JSON.stringify({ err, code, lang }));
}
export function getTwoslashOptions({ linkMap } = { linkMap: new Map() }) {
  return {
    onTwoslashError,
    onShikiError,
...
export function parseLineComment(line) {
  line = line.trim();
...
function checkIsExternalLink(href) {
