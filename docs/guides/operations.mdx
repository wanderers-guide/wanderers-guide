...
Content grants may nest, but a grant cannot lead back to the same content on its active
ancestor path. Reusing a feat in separate branches is allowed. Cycles report the content
IDs involved and fail the calculation; the sheet retains its last completed calculation.
The engine also limits nesting to 64 levels, forward work to 100,000 operations across
all passes, retained variable effects to 200,000, and removal reconstruction to 1,000,000 replay steps. These limits also apply
when Web Workers are unavailable. Split unusually large homebrew operation graphs and
correct self-grants instead of relying on partial calculation results.
...
`LANGUAGE_NAMES` and `LANGUAGE_IDS` are explicit final overrides. Set either to a JSON array of strings; the engine resolves the matching records and replaces both lists after normal grants and removals. An empty array removes all languages, including ancestry grants. Without an override, normal language operations retain their existing behavior.

```jsonc
// A creature that speaks no languages, even if its ancestry grants some
{ "id": "...", "type": "setValue", "data": { "variable": "LANGUAGE_NAMES", "value": "[]" } }

// Replace the entire list with specified language IDs
{ "id": "...", "type": "setValue", "data": { "variable": "LANGUAGE_IDS", "value": "[\"81\"]" } }
```

Native array values are also supported. Unknown or ambiguous names, unknown IDs, and malformed lists produce an error without partially replacing either list. Use IDs when several sources define the same name. Deferred bindings and the final sheet see the replacement; conditionals still evaluate during their normal operation passes.
...
For one check or damage roll, applicable modifier categories share one stacking
calculation. The highest bonus and worst penalty of each type apply separately;
untyped modifiers add together. For example, Frightened 1 and Stupefied 2 impose
one status penalty of 2 on an Intelligence spell attack. Clumsy applies to
Dexterity attacks, while Enfeebled applies to Strength attacks and damage. Neither
automatically penalizes an Intelligence spell attack. Conditional text remains a
displayed rider rather than an unconditional numeric modifier.

Armor and an active invested potency rune form one item bonus to AC, which then
competes with other item bonuses. Unburdened Iron ignores the armor speed penalty
and reduces just one eligible other speed penalty by up to 5 feet, then applies
normal typed stacking to the remaining penalties.

Effective conditions use the strongest explicit or derived value regardless of
selection order. Derived conditions do not replace their independently selected
sources: removing Encumbered also removes its derived Clumsy 1, but keeps an
independently selected Clumsy condition.
...
Removal revokes the matching grants that have already run, including their nested grants,
variable changes, typed bonuses, and deferred bindings or language overrides. Effects from
independent sources remain, even if their equal or lower values were previously hidden.
The same ownership applies to selected abilities, class features, and creature abilities.
Removing a mode also deactivates it for this calculation.

A later explicit grant can grant the content again. Removing a source that performed a
removal also revokes that removal, restoring independent earlier grants. The engine
rebuilds the remaining write intentions in order; it does not subtract the removed
source's final numeric difference. Conditions retain the normal three-pass evaluation
order, rather than repeatedly reevaluating to a fixed point.
...
`removeSpell` removes all existing grants of that spell ID, including each casting-source
record in `SPELL_DATA` and innate spell proficiency granted by that spell. Other spells
and independently granted casting proficiency remain. Names stay synchronized with IDs,
including when two different spell records share a name. Spell-slot pools belong to their
casting source and are not removed with an individual spell.
...
Successful grants also add the trait's canonical name to the `TRAIT_NAMES` list variable for conditionals.
...
Proficiency comparisons use `U`, `T`, `E`, `M`, or `L`. The editor saves its initial
Untrained choice as `U`. Older proficiency checks with an empty string are interpreted
as that same displayed default; other invalid rank values are not silently converted.

Boolean comparisons use the strings `TRUE` and `FALSE`. Selecting a boolean variable
or value type saves the editor's initial True choice as `TRUE`. Existing empty or
non-`TRUE` values retain their false comparison and display False; opening the editor
...
Ancestry feat filters also admit the existing **All Ancestries** trait, while preserving level, required-trait, and feat-type constraints. Archetype skill feats belong in skill feat choices: ordinary class feat and Free Archetype choices exclude them. A filter that explicitly requests the Skill trait can still select them.
...
Item injections appear in both catalog and inventory descriptions. They use the store of the character or companion whose drawer is open, so one entity’s modifications do not appear on another.
