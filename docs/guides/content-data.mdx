...
The character list requires the same signed-in account that requested it. If its
authentication session becomes unavailable or changes, the list shows a load error
with Retry instead of treating an anonymous response as an empty account.
