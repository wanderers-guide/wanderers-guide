...
1. **Start the frontend** with `npm --prefix frontend run dev`. The command catalog at `.agents/launch.json` also records the docs preview command. These are ordinary npm commands; use the available terminal tools to start them.
2. **Open the app** in the available browser tools. The public sheet `/sheet/142809` and drawer URL patterns below support read-only verification without login.
3. **Inspect rendered behavior** using the browser's accessibility tree or DOM snapshot for text and structure, computed styles for layout questions, and screenshots for visual verification. Check browser console errors and the server log.
4. **Validate mobile layout** as well as desktop when a user-facing change affects layout or controls.

When something fails, inspect the rendered DOM before changing code again. Use the browser tool's documented APIs; load its browser/local-development guidance when needed.
