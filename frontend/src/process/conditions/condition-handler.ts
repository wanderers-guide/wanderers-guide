...
export function getConditionByName(
  ...
  const foundCondition = _.cloneDeep(
    CONDITIONS.find((condition) => condition.name === name)
  );
  if (foundCondition) {
...
export function compiledConditions(conditions: Condition[]): Condition[] {
  ...
  // Remove duplicates
  return _.uniqWith(newConditions, _.isEqual).sort((a, b) =>
...
function applyCondition(id: StoreID, condition: Condition) {
  if (condition.name === "Blinded") {
    ...
    const visionSenses =
      preciseSenses?.value.filter((sense) => sense.includes("VISION")) ?? [];
  ...
  if (condition.name === "Clumsy") {
    ...
    const dexSaves = getAllSaveVariables(id).filter(
      (save) => save.value.attribute === "ATTRIBUTE_DEX"
    );
    ...
    const dexSkills = getAllSkillVariables(id).filter(
      (skill) => skill.value.attribute === "ATTRIBUTE_DEX"
    );
