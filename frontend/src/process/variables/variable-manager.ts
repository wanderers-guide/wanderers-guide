  ...
  if (isVariableProf(variable)) {
    if (isProficiencyValue(amount) || isExtendedProficiencyValue(amount)) {
      ...
      if (isProficiencyType(value)) {
        ...
        if (value === "1") {
          variable.value.value =
            nextProficiencyType(variable.value.value) ?? variable.value.value;
        } else if (value === "-1") {
          variable.value.value =
            prevProficiencyType(variable.value.value) ?? variable.value.value;
        } else {
          throwError(
            `Invalid adjust amount for extended prof: ${name}, ${JSON.stringify(
              amount
            )}`
          );
        }
      ...
      throwError(
        `Invalid adjust amount for prof: ${name}, ${JSON.stringify(amount)}`
      );
    }
    ...
    throwError(
      `Invalid adjust amount for variable: ${name}, ${JSON.stringify(amount)}`
    );
  }
