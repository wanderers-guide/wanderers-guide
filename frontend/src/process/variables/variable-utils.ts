  ...
  if (type === "prof") {
    return {
      ...
      value: {
        ...
        attribute: isProficiencyValue(defaultValue)
...
export function nextProficiencyType(
...
export function prevProficiencyType(
  profType: ProficiencyType
): ProficiencyType | null {
...
export function findVariable<T = Variable>(
  ...
  const variable = Object.values(getVariables(id)).find(
    (variable) =>
      variable.type === type && variable.name.endsWith(`_${VAR_FORMATTED}`)
  );
...
export function isExtendedProficiencyValue(
  value: any
): value is ExtendedProficiencyValue {
  return (
...
export function isAttributeValue(value: any): value is AttributeValue {
  return (
    ...
    (isBoolean(value?.partial) || value?.partial === undefined)
  );
}
export function isProficiencyType(value?: any): value is ProficiencyType {
  return ["U", "T", "E", "M", "L"].includes(value ?? "");
}
export function isExtendedProficiencyType(
  value?: string
): value is ExtendedProficiencyType {
...
export function isVariableListStr(
  value: Variable | any
): value is VariableListStr {
