...
import { ItemMetaGroupArmor } from '@schemas/content';
import { cloneDeep } from 'lodash-es';

interface ArmorSpecialization {
  group: ItemMetaGroupArmor;
  name: string;
  description: string;
}

const PATHFINDER_SPECIALIZATIONS: ArmorSpecialization[] = [
  {
    group: 'chain',
    name: 'Chain',
...
const STARFINDER_SPECIALIZATIONS: ArmorSpecialization[] = [
  {
    group: 'cloth',
    name: 'Cloth',
...
export function getArmorSpecialization(group: ItemMetaGroupArmor) {
  return getArmorSpecializations().find((s) => s.group === group);
}

export function getArmorSpecializations(includeAll = false): ArmorSpecialization[] {
