...
export function StatAttrDrawerTitle(props: {
  data: { attributeName?: string };
}) {
  return (
    <>
          ...
          <Box>
            <Title order={3}>
              {props.data.attributeName
                ? `Attribute: ${variableNameToLabel(props.data.attributeName)}`
                : "Attributes"}
            </Title>
          </Box>
        </Group>
...
export function StatAttrDrawerContent(props: {
  data: { attributeName?: string };
}) {
  ...
  for (const attribute of attributes) {
    ...
    for (const [index, record] of history.entries()) {
      ...
      const existingValue =
        sourceRecords[record.source][attributes.indexOf(attribute)] ?? 0;
    ...
    <Table.Tr>
      {attributes.map((attribute, index) => (
        <Table.Th key={index} ta="center">
          ...
          <Text
            c="gray.0"
            td={attribute.value.partial ? "underline" : undefined}
            span
          >
