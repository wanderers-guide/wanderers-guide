...
export function StatProfDrawerTitle(props: {
  data: { variableName: string; isDC?: boolean };
}) {
  const variable = getVariable<VariableProf>(
    "CHARACTER",
...
export function StatProfDrawerContent(props: {
  data: { variableName: string; isDC?: boolean };
}) {
  const variable = getVariable<VariableProf>(
    "CHARACTER",
  ...
  for (const hist of history) {
    const from = isProficiencyValue(hist.from)
      ...
      : hist.from;
    const to = isProficiencyValue(hist.to)
      ? proficiencyTypeToLabel(hist.to.value)
      : hist.to;
    timeline.push({
      type: "ADJUSTMENT",
      title: `${from} → ${to}`,
    ...
    <Box>
      <Accordion variant="separated" defaultValue="">
          ...
          <Accordion.Item value="breakdown">
            ...
            <Accordion.Panel>
              <Group gap={8} wrap="nowrap" align="center">
                ...
                >
                  ...
                  <HoverCard.Dropdown py={5} px={10}>
                    <Text c="gray.0" size="xs">
                      ...
                      in this proficiency, resulting in a{" "}
                      {sign(getProficiencyTypeValue(variable.value.value))}{" "}
                      bonus.
                    </Text>
                  </HoverCard.Dropdown>
                </HoverCard>
                    ...
                    <Text c="gray.0" size="xs">
                      {variable.value.value === "U" ? (
                        <>
                          ...
                          in this proficiency, you don't add your level.
                        </>
                      ) : (
                        <>
                          Because you're{" "}
                          ...
                          in this proficiency, you add your level.
                        </>
                      )}
                    </Text>
                  </HoverCard.Dropdown>
                    ...
                    >
                      ...
                      <HoverCard.Dropdown py={5} px={10}>
                        <Text c="gray.0" size="xs">
                          ...
                          {variableNameToLabel(variable.value.attribute ?? "")}{" "}
                          attribute, so you add your{" "}
                          {variableNameToLabel(variable.value.attribute ?? "")}{" "}
                          modifier.
                        </Text>
                      </HoverCard.Dropdown>
                ...
                {[...parts.breakdown.bonuses.entries()].map(
                  ([key, bonus], index) => (
                    <>
                      +
                      <HoverCard
                      ...
                      >
                        ...
                        <HoverCard.Dropdown py={5} px={10}>
                          <Text c="gray.0" size="xs">
                            Your {key} bonus. Use the highest from the
                            following:
                            <Divider pb={5} />
                            <List size="xs">
                ...
                {parts.hasConditionals && (
                  <>
                    ...
                    >
                      ...
                      <HoverCard.Dropdown py={5} px={10}>
                        <Text c="gray.0" size="xs">
                          You have conditional bonuses! These will only apply
                          situationally:
                          <Divider pb={5} />
                          <List size="xs">
        ...
        <Accordion.Item value="timeline">
          <Accordion.Control icon={<IconTimeline size="1rem" />}>
            Timeline
          </Accordion.Control>
          <Accordion.Panel>
            <Box>
    ...
    queryFn: async ({ queryKey }) => {
      ...
      return abilityBlocks.filter((block) => {
        ...
        return (
          ...
          (convertedSkills.includes("SKILL_LORE") &&
            variableName.startsWith("SKILL_LORE_"))
        );
      });
    },
  });
