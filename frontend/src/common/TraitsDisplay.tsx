...
export default function TraitsDisplay(props: {
  ...
  const { data: traits } = useQuery({
    ...
    queryFn: async ({ queryKey }) => {
      ...
      return (await fetchTraits(props.traitIds)).sort((a, b) =>
  ...
  return (
    <Group gap={3} justify={props.justify}>
      ...
      {props.broken && (
        <BrokenDisplay interactable={props.interactable} size={props.size} />
      )}
          ...
          <HoverCard.Target>
            <Badge
              ...
              styles={{
                root: {
                  // @ts-ignore
                  "--badge-dot-size": trait.meta_data?.important
                    ? undefined
                    : 0,
...
export function BrokenDisplay(props: {
  interactable?: boolean;
  size?: MantineSize;
}) {
  const theme = useMantineTheme();
...
export function ShoddyDisplay(props: {
  interactable?: boolean;
  size?: MantineSize;
}) {
  const name = "Shoddy";
...
export function TraitOverview(props: {
  name: string;
  description: string;
  important: boolean;
}) {
