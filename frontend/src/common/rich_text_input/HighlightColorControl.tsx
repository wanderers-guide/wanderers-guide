...
export const HighlightColorControl = forwardRef<HTMLButtonElement, HighlightColorControlProps>((props, ref) => {
  ...
  const handleChange = (value: string, shouldClose = true) => {
    (editor?.chain() as any).focus().setMark('highlight', { color: value }).run();
    shouldClose && close();
  };

  const handleClear = () => {
    (editor?.chain() as any).focus().unsetMark('highlight').run();
    close();
  };
    ...
    <Popover opened={opened} withinPortal trapFocus onClose={close} {...popoverProps}>
      <Popover.Target>
        <RichTextEditorControl
          ...
          onClick={toggle}
        >
          <BiHighlight color={currentColor} size={14} />
        </RichTextEditorControl>
      </Popover.Target>
...
HighlightColorControl.displayName = 'HighlightColorControl';
